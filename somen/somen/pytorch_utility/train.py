...
def train(
    ...
    find_unused_parameters: bool = params.get("find_unused_parameters", True)
    ...
    sampler_seed: int = params.get("sampler_seed", 0)
    ...
    maximize: Union[bool, Sequence[bool]] = params.get("maximize", False)
    ...
    if train_sampler is None and distributed:
        train_sampler = torch.utils.data.distributed.DistributedSampler(train_set, seed=sampler_seed)
    else:
    ...
    if distributed:
        ...
        update_fn = get_update_fn(
            DistributedDataParallel(model, device_ids=[device], find_unused_parameters=find_unused_parameters),
            optimizer,
            loss_fn,
            device,
            label_indices,
    ...
    if valid_sets is not None:
        ...
        if isinstance(maximize, bool):
            maximize = [maximize] * len(metric_functions)
        else:
            if len(maximize) != len(metric_functions):
                raise ValueError
        ...
        for valid_set, valid_name in zip(valid_sets, valid_names):
            ...
            if take_best_value_snapshot:
                for metric_name, maximize_this in zip(metric_functions.keys(), maximize):
                    extensions.append(
                        BestValueSnapshot(
                            model,
                            working_dir / f"{valid_name}_{metric_name}_best.pth",
                            valid_name + "/" + metric_name,
                            maximize_this,
                            eval_event,
                        )
                    )
    ...
    def _extension_iteration_completed(engine: Engine) -> None:
        ...
        if isinstance(output, dict):
            engine.state.metrics["observation"] = {f"train/{key}": value for key, value in output.items()}
        else:
            engine.state.metrics["observation"] = {"train/loss": output}
