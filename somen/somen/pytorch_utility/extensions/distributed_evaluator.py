...
class DistributedEvaluator(Extension):
    ...
    def __call__(self, engine: Engine) -> None:
        ...
        for key, value in total.items():
            tensor = torch.from_numpy(np.array(value)).to(self.device)
            torch.distributed.reduce(tensor, 0, torch.distributed.ReduceOp.SUM)
            if self.rank == 0:
                total[key] = tensor.item()

        for key, value in divisor.items():
            tensor = torch.from_numpy(np.array(value)).to(self.device)
            torch.distributed.reduce(tensor, 0, torch.distributed.ReduceOp.SUM)
            if self.rank == 0:
                divisor[key] = tensor.item()
